...
def producingCandidateFixesCombinations(xmlRoot, xmlElem, attributeName, filePath, appProjectRootPath, appProjectName,
    ...
    print(optimalAttrValue)
    if str(optimalAttrValue) == '{}':
        print("no issue-fixing attributes were found!!")
        exit(0)
...
def test_case(deviceId, apkPath, mode):
    ...
    try:
        ...
        bounds = xmltestcasedriver.test_case_driver(test_case_name, deviceId, apkPath)
        global before1bounds
        if mode == "before1":
            before1bounds = bounds
        if mode == "before2":
            if bounds == before1bounds and bounds != ():
                print("Same bounds, XFix cannot fix")
                exit(0)
