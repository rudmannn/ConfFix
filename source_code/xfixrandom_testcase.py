...
bboundsBase = None
bboundsTest = None
# apkPath = "/Users/huanghuaxun/Downloads/app-debug/dist/signed.apk"
optimalAttrValue = {}
optimalFitness = {}
        ...
        if len(dimenAttrValues) > 0:
            ...
            for dimenAttrValue in dimenAttrValues:
                ...
                try:
                    ...
                    fitness = xmlfitness.getFitnessByDOM(resId, before1, after)
                ...
                if fitness < suboptimalFitness:
                    ...
                    print("suboptimalFitness:: " + str(suboptimalFitness))
...
def producingCandidateFixes(xmlRoot, xmlElem, attributeName, filePath, appProjectRootPath,appProjectName, deviceId):
    ...
    if dataformat == "dimension":
        ...
        for attribute in attributes:
            if str(attribute).__contains__("res-auto") == True:
                continue
...
def producingBaselines(xmlRoot, xmlElem, attributeNames, filePath, appProjectRootPath, appProjectName, deviceId):
    ...
    fitness = xmlfitness.getFitnessByDOM(resId, before1, before2)
    print("originalFitness::"+str(fitness))
    if fitness == 0.0:
        print("There is no inconsistent layout bounds. Terminate!")
        exit(0)
...
def generateCombinationByImprovement(xmlRoot, xmlElem, attributeName, filePath, appProjectRootPath, appProjectName,
    ...
    for i in range(0,10):
        ...
        if fitness < optimalCombinationFitness:
            ...
            print("suboptimal fitness score:: "+str(optimalCombinationFitness))
...
def producingCandidateFixesCombinations(xmlRoot, xmlElem, attributeName, filePath, appProjectRootPath, appProjectName,
    ...
    originalFitnessScore = xmlfitness.getFitnessByDOM(resId, before1, before2)
...
def test_case(deviceId, apkPath, mode):
    ...
    try:
        ...
        try:
            cmd = "/data/hhuangas/android_sdk/platform-tools/adb -s "+deviceId+" shell am instrument -w -m -e debug false -e class '"+testMethod+"' "+testApkPackage+"/androidx.test.runner.AndroidJUnitRunner"
            print("cmd:::"+cmd)
        ...
        if mode == "before1":
            cmd = "/data/hhuangas/android_sdk/platform-tools/adb -s "+deviceId+" shell \"run-as "+appId+" cat /data/user/0/"+appId+"/files/bounds_out.txt\" > "+before1
            print("cmd: "+cmd)
        elif mode == "before2":
            cmd = "/data/hhuangas/android_sdk/platform-tools/adb -s "+deviceId+" shell \"run-as "+appId+" cat /data/user/0/"+appId+"/files/bounds_out.txt\" > "+before2
            print("cmd: " + cmd)
        elif mode == "after":
            cmd = "/data/hhuangas/android_sdk/platform-tools/adb -s "+deviceId+" shell \"run-as "+appId+" cat /data/user/0/"+appId+"/files/bounds_out.txt\" > "+after
            print("cmd: " + cmd)
...
if __name__ == '__main__':
    ...
    hasAndroidNameSpace = False
    for attribute in attributes:
        if str(attribute).__contains__("res-auto") == True:
            continue
        else:
            str1 = attribute.split('}')[1]
            if str1 == issueInducingAttribute:
                continue
            hasAndroidNameSpace = True
            break

    if hasAndroidNameSpace == False:
        print("No issue-fixing attributes were found, exit.")
        exit(0)
